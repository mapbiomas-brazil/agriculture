...
var input = 'users/your_username/MAPBIOMAS/C6/AGRICULTURE/COFFEE/RESULTS/RAW'

// set the path for the filtered result:
var output = 'users/your_username/MAPBIOMAS/C6/AGRICULTURE/COFFEE/RESULTS/TEMPORAL_SPATIAL_FILTERED'
...
var firstYear = filteredCollection.filterMetadata('year', 'equals', startYear + 1).first().set('year', startYear);
var lastYear = filteredCollection.filterMetadata('year', 'equals', endYear - 1).first().set('year', endYear);

filteredCollection = filteredCollection
  .filter(ee.Filter.inList('year', [startYear, endYear]).not())
  .merge(ee.ImageCollection([firstYear, lastYear]))
  .sort('year')
...
Export.image.toAsset({
  image: filtered.byte(),
  description: 'COFFEE_TEMPORAL_SPATIAL_FILTER',
  assetId: output,
